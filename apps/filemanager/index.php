...
$favicon_path = 'https://raw.githubusercontent.com/neoistone/neoistone/master/logo150.png';
...
$public_ip = file_get_contents('https://icanhazip.com');
$ip_whitelist = array(
...
if ( !defined( 'FM_SESSION_ID')) {
    define('FM_SESSION_ID', 'nspanel_filemanager');
}
...
if(!empty($_SESSION[FM_SESSION_ID]['logged'])) {
    $root_path = base64_decode($_SESSION[FM_SESSION_ID]['ssid']);
}
...
if (isset($_SESSION[FM_SESSION_ID]['logged'])) {
    ...
    // Logging In
    sleep(1);
    $con = mysqli_connect('localhost','root',trim(file_get_contents('/opt/sneoistone')),'panel');
    $sql = "SELECT * FROM users WHERE username = '".mysqli_real_escape_string($con,$_POST['fm_usr'])."';";
    $result = mysqli_query($con,$sql);
    $row = mysqli_fetch_array($result,MYSQLI_ASSOC);

    if(mysqli_num_rows($result) == 1) {
       if ($row['status'] == 'active') {
       	   if ((sha1($_POST['fm_pwd']) == $row['passsword'])) {
       	   	   $_SESSION[FM_SESSION_ID]['logged'] = $_POST['fm_usr'];
       	   	   $_SESSION[FM_SESSION_ID]['ssid'] = base64_encode($row['home_dir']);
               fm_set_msg(lng('You are logged in'));
               fm_redirect(FM_SELF_URL . '?p=');
       	   } else {
               unset($_SESSION[FM_SESSION_ID]['logged']);
              fm_set_msg(lng('Password Incorrect'), 'error');
              fm_redirect(FM_SELF_URL);
    	   }
       }else {
       	 unset($_SESSION[FM_SESSION_ID]['logged']);
         fm_set_msg(lng('User Account suspend'), 'error');
         fm_redirect(FM_SELF_URL);
       }
    } else {
       unset($_SESSION[FM_SESSION_ID]['logged']);
       fm_set_msg(lng('User Not Found'), 'error');
       fm_redirect(FM_SELF_URL);
    }
} else {
    // Form
            ...
            <div class="row justify-content-md-center h-100">
                <div class="card-wrapper">
                            ...
                            <form class="form-signin" action="" method="post" autocomplete="off">
                                <div class="form-group">
                                    <div class="text-center">
                                        <img src="https://raw.githubusercontent.com/neoistone/neoistone/master/logo.png" style="width: 100%; height: 80px;" class="text-center">
                                    </div>
                                </div>
                                <hr />
                                <div class="form-group">
                    ...
                    <div class="footer text-center">
                        &mdash;&mdash; &copy; All Copyright Reseverd By
                        <a href="https://neoistone.com/" target="_blank" class="text-muted" >Neoistone</a> &mdash;&mdash;
                    </div>
                </div>
            </div>
        </div>
            ...
            foreach ($folders as $f) {
                ...
                <tr>
                    ...
                    <td class="inline-actions"><?php if (!FM_READONLY): ?>
                        ...
                        <?php endif; ?>
                        <a title="<?php echo lng('DirectLink')?>" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/filemanger/index.php?p=' . FM_PATH : '') . '/' . $f . '/') ?>" target="_blank"><i class="fa fa-link" aria-hidden="true"></i></a>
                    </td>
                </tr>
                <?php
                flush();
...
<script>
    ...
    //TFM Config
    window.curi = "<?php echo 'https://'.gethostname().':7081/filemanager/filemanager.json'; ?>", window.config = null;
